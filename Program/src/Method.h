...
*************************************************************************************/
void CreatePoolSolutions(const Problem& problem, const int sizePool)
{
    #pragma omp critical
    {
        for (int i = 0; i < sizePool; i++){
            CreateInitialSolutions(pool[i], problem.dimension);
            pool[i].ofv = problem.decode(pool[i]);
        ...
        for (int i = sizePool-1; i >0 ; i--){
            if (pool[i].ofv == pool[i-1].ofv){
                for (int j=0; j<0.2*problem.dimension; j++){
                    int pos = irandomico(0, problem.dimension-1);
...
*************************************************************************************/
void NelderMeadSearch(TSol &x1, const Problem& problem)
{
    int improved = 0;
    int improvedX1 = 0;
    TSol x1Origem = x1;
    ...
    x0 = Blending(x1, x2, 1, problem.dimension);
    x0.ofv = problem.decode(x0);
    if (x0.ofv < xBest.ofv) {
        xBest = x0;
        improved = 1;
    }
    ...
    int maxIter = problem.dimension*exp(-2);
    while (iter_count <= maxIter)
    {
        int shrink = 0;

        // reflection point (r)
        x_r = Blending(x0, x3, -1, problem.dimension);
        x_r.ofv = problem.decode(x_r);
        ...
        {
            // expansion point (e)
            x_e = Blending(x_r, x0, -1, problem.dimension);
            x_e.ofv = problem.decode(x_e);
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // contraction point (c)
                    x_c = Blending(x_r, x0, 1, problem.dimension);
                    x_c.ofv = problem.decode(x_c);
                ...
                {
                    // contraction point (c)
                    x_c = Blending(x0, x3, 1, problem.dimension);
                    x_c.ofv = problem.decode(x_c);
                    if (x_c.ofv < xBest.ofv) {
                        xBest = x_c;
                        improved = 1;
        ...
        if (shrink) {
            x2 = Blending(x1, x2, 1, problem.dimension);
            x2.ofv = problem.decode(x2);
            if (x2.ofv < xBest.ofv) {
                xBest = x2;
                improved = 1;
                improvedX1 = 1;
            ...
            x3 = Blending(x1, x3, 1, problem.dimension);
            x3.ofv = problem.decode(x3);
            if (x3.ofv < xBest.ofv) {
                xBest = x3;
                improved = 1;
                improvedX1 = 1;
...
*************************************************************************************/
void SwapLS(TSol &s, const Problem& problem, const int &strategy, std::vector<int> &RKorder)
{
    ...
    if (strategy == 1){
        TSol sBest = s;
        for(int i = 0; i < (problem.dimension-1)*rate; i++) {
            for(int j = i+1; j < problem.dimension*rate; j++) {
...
*************************************************************************************/
void InvertLS(TSol &s, const Problem& problem, const int &strategy, std::vector<int> &RKorder)
{
    // define a random order for the neighors
    std::shuffle(RKorder.begin(), RKorder.end(),rng);
    ...
    {
        TSol sBest = s;
        for(int i = 0; i < problem.dimension*rate; i++) {
...
*************************************************************************************/
void FareyLS(TSol &s, const Problem& problem, const int &strategy, std::vector<int> &RKorder)
{
    // define a random order for the neighors
    std::shuffle (RKorder.begin(), RKorder.end(),rng);
...
*************************************************************************************/
void RVND(TSol &s, const Problem& problem, const int &strategy, std::vector<int> &RKorder)
{
    ...
    while (!NSL.empty())
    {
        ...
        {
            case 1:
                SwapLS(s, problem, strategy, RKorder);
                break;

            case 2:
                InvertLS(s, problem, strategy, RKorder);
                break;

            case 3:
                NelderMeadSearch(s, problem);
                break;

            case 4:
                FareyLS(s, problem, strategy, RKorder);
                break;
