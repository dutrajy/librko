...
double Decoder(TSol s) // best + redudance
{
    // create an initial list of candidates
    std::vector <int> sol(n);  
    for (int j = 0; j < n; j++){ sol[j] = j;}

    // for (int j = 0; j < n; j++){ printf("%.3lf-%d \n",s.rk[j], sol[j]);}
    ...
    while (numCobertura < numTriples)
    {
        ...
        for (int i=0; i<(int)varCover[sol[k]].size(); i++)
        {
            // verificar se a linha ainda nao foi coberta
            if (cover[varCover[sol[k]][i]] == 0)
            {
                // atualizar a cobertura da linha
                cover[varCover[sol[k]][i]] = 1;
                numCobertura++;
    ...
    {
        int redundante = 1;
        for (int j=0; j<(int)varCover[sol[i]].size(); j++)
        {
            if (cover[varCover[sol[i]][j]] == 1)
            {
                redundante = 0;
                break;
            }
